...
const CommonTag = () => {
  ...
  const setTag = (flag, item, index) => {
    // flag用于判断是否为选中的tag
    if (index == 0 && currentMenu.name == undefined) {
      return (
        <Tag key={item.name} color="#55acee">
          {item.label}
        </Tag>
      );
    }
    return flag ? (
      ...
      <Tag key={item.name} onClick={() => handleChange(item, index)}>
        {item.label}
      </Tag>
    );
  };
  return (
    <Space className="common-tag" size={[0, 8]} wrap>
      {(currentMenu.name || tabsList.length > 1) &&
